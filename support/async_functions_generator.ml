...
let main () =
  ...
  | "ml" ->
     let ml_out = open_out "src/async_functions_generated.ml" in
     let ml_fmt = Format.formatter_of_out_channel ml_out in
     Cstubs.write_ml ~concurrency ml_fmt ~prefix (module Async_function_description.Functions);
     Format.pp_print_flush ml_fmt ();
     close_out ml_out
  | "c" ->
     let c_out = open_out "src/async_functions_stubs.c" in
     let c_fmt = Format.formatter_of_out_channel c_out in
     Format.fprintf c_fmt "%s@\n" c_headers;
     Cstubs.write_c ~concurrency c_fmt ~prefix (module Async_function_description.Functions);
